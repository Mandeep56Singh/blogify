...
//    public ResponseEntity<Response<ImageDto>> saveImage(
//            @RequestParam("file") MultipartFile file
//    ) throws IOException {
//
//        Response<ImageDto> imageDto = imageService.uploadImage(file);
//        if (!imageDto.success()) {
...
//    ) {
//        Optional<Response<ResponsePayload>> violationError = validator.validate(new StringIdWrapper(id));
//        if (violationError.isPresent()) {
//            return new ResponseEntity<>(violationError, violationError.get().error().status());
//        }
//
//        Response<ImageResourceDto> responseDto = imageService.loadImage(id);
//
